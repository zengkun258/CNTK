...
namespace CNTK
{
    ...
    // Todo: Move into NextAutoGeneratedDynamicAxis() after upgrade to VS2015.
    /*static*/ const std::wstring CompositeFunction::s_autoGeneratedDynamicAxisNamePrefix = L"autoGeneratedDynamicAxis_";

    // Todo: Move into Deserialize() after upgrade to VS2015
    static const vector<std::wstring> s_requiredDictionaryKeysInCompositeFunction = {typeKey, rootKey, nameKey, uidKey, inputsKey, functionsKey};
    /*static*/ FunctionPtr CompositeFunction::Deserialize(const Dictionary& dict, const CNTK::DeviceDescriptor& device)
    {
        size_t version = ValidateDictionary<CompositeFunction>(dict, s_requiredDictionaryKeysInCompositeFunction, s_compositeFunctionTypeValue, s_serializationVersion);
